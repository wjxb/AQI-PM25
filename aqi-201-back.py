...
citiesGeoFile = 'china_cities_geo.json'
aqiGeoFile= '/home/wntime/aicc-view/js/aqiGeo.js'
...
def updateHistoryCityJson(old_time_point_key,newData):
	...
	for x in range(len(jsonObj)):
		...
		try:
			...
			dataList = [jsonObj[x]]
			#第一次保存的时候，加入经纬度信息
			coordinate = getcoordinate(baiduUrlAddressInCity,jsonObj[x]["area"],'')
			coordinateObj = json.loads(coordinate)
			firstTimePointDict = {"AQI":dataList,"latitude":coordinateObj['result']['location']['lat'],"longitude":coordinateObj['result']['location']['lng']}
		...
		if(coordinate==''):
                ...
                if(coordinateObj['status']==1):
                       	continue
		result["AQI"] = dictOld
		result["latitude"]=coordinateObj['result']['location']['lat']
		result["longitude"]=coordinateObj['result']['location']['lng']
...
if __name__ == '__main__':
	...
	#jsonData = aqiJson()
	jsonData = '[{"aqi": 19,"area": "天鞍山","pm2_5":8,"time_point":"2012-07-28T09:00:07Z"}]'
	#	jsonData = aqiJson()
	jsonData = '[{"aqi": 30,"area": "广州","pm2_5":8,"time_point":"2012-05-28T09:00:06Z"},{"aqi": 17,"area": "北京","pm2_5": 10,"time_point": "2012-05-28T09:00:06Z"}]'
